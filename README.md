...
標準ライブラリのHTTPサーバーを用いた簡易的なAPIを同梱しています。次のコマンドでサーバーを起動できます。

```bash
python web_app.py
```

起動後は以下のエンドポイントが利用できます。

- `/works?author_url=<URL>`: 指定した著者ページから作品の URL 一覧を JSON で返します。
- `/work?url=<URL>`: 作品ページから本文を抽出し JSON で返します。
